...
import { AddColumn, Migration } from './type';
  ...
  for (const i in migrations) {
      ...
      funcArray.push(async () => {
        ...
        if (!exists) {
          return db.schema.createTable(<string>migration.table, t => {
            for (const i in migration.column) {
    ...
    if (migration.opt === Operation.dropColumn) {
      funcArray.push(async () => {
        ...
        if (exists) {
          return db.schema.table(<string>migration.table, t => t.renameColumn(
              <string>migration.from_column,
    ...
    if (migration.opt === Operation.raw) {
      funcArray.push(async () => db.schema.raw(<string>migration.sql));
    }
  }
